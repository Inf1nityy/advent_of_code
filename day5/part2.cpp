...
bool customComparator(int a, int b) {
    if (pageOrdering.find(a) != pageOrdering.end()) {
        if (std::find(pageOrdering[a].begin(), pageOrdering[a].end(), b) != pageOrdering[a].end()) {
            return true;
        }
    }

    if (pageOrdering.find(b) != pageOrdering.end()) {
        if (std::find(pageOrdering[b].begin(), pageOrdering[b].end(), a) != pageOrdering[b].end()) {
            return false;
        }
    }
